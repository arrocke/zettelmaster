...
interface EntityData { str?: string }
...
describe('equals', () => {
	test('returns true if Entities have the same identity', () => {
		...
		const data = { str: 'hello world' }
		const entity = new TestEntity(id, data)
		expect(entity.equals(new TestEntity(id, { str: 'something different' }))).toEqual(true)
		expect(entity.equals(new TestEntity(Identifier.generate(), data))).toEqual(false)
	})
})
